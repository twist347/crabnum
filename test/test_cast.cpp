...
TEST(Cast, AsRawIntToInt) {
    constexpr cn::Num<std::int32_t> a{42};
...
TEST(Cast, AsRawIntToFloat) {
    constexpr cn::Num<std::int32_t> a{7};
    EXPECT_EQ(a.as_raw<float>(), 7.0f);
    EXPECT_EQ(a.as_raw<double>(), 7.0);
}

TEST(Cast, AsRawFloatToInt) {
    constexpr cn::Num<double> a{3.14};
    EXPECT_EQ(a.as_raw<std::int32_t>(), 3);
}

TEST(Cast, AsNumToNum) {
    constexpr cn::i32 a{42};
    constexpr auto b = a.as<cn::i64>();
    static_assert(std::is_same_v<std::remove_cv_t<decltype(b)>, cn::i64>);
    EXPECT_EQ(b.value(), std::int64_t{42});
}

TEST(Cast, AsNarrow) {
    constexpr cn::i32 a{300};
    constexpr auto b = a.as<cn::i8>();
    static_assert(std::is_same_v<std::remove_cv_t<decltype(b)>, cn::i8>);
    EXPECT_EQ(b.value(), static_cast<std::int8_t>(300));
}

TEST(Cast, AsIntToFloat) {
    constexpr cn::i32 a{7};
    constexpr auto b = a.as<cn::f64>();
    static_assert(std::is_same_v<std::remove_cv_t<decltype(b)>, cn::f64>);
...
TEST(Cast, TryAsWideningOk) {
    constexpr auto res = cn::i8{42}.try_as<std::int32_t>();
    ASSERT_TRUE(res.has_value());
    EXPECT_EQ(res->value(), 42);
}

TEST(Cast, TryAsNarrowingOk) {
    constexpr auto res = cn::i32{42}.try_as<std::int8_t>();
    ASSERT_TRUE(res.has_value());
    EXPECT_EQ(res->value(), std::int8_t{42});
}

TEST(Cast, TryAsNarrowingFail) {
    constexpr auto res = cn::i32{300}.try_as<std::int8_t>();
    EXPECT_FALSE(res.has_value());
}

TEST(Cast, TryAsSignedToUnsignedOk) {
    constexpr auto res = cn::i32{42}.try_as<std::uint32_t>();
    ASSERT_TRUE(res.has_value());
    EXPECT_EQ(res->value(), std::uint32_t{42});
}

TEST(Cast, TryAsSignedToUnsignedNegative) {
    constexpr auto res = cn::i32{-1}.try_as<std::uint32_t>();
    EXPECT_FALSE(res.has_value());
}

TEST(Cast, TryAsUnsignedToSignedOk) {
    constexpr auto res = cn::u32{42}.try_as<std::int32_t>();
    ASSERT_TRUE(res.has_value());
    EXPECT_EQ(res->value(), 42);
}

TEST(Cast, TryAsUnsignedToSignedFail) {
    constexpr auto res = cn::u32{3'000'000'000u}.try_as<std::int32_t>();
    EXPECT_FALSE(res.has_value());
}

TEST(Cast, TryAsIntToFloatOk) {
    constexpr auto res = cn::i32{42}.try_as<double>();
    ASSERT_TRUE(res.has_value());
...
TEST(Cast, TryAsIntToFloatPrecisionLoss) {
    ...
    constexpr auto res = cn::i64{(1LL << 53) + 1}.try_as<double>();
    EXPECT_FALSE(res.has_value());
}

TEST(Cast, TryAsFloatToIntOk) {
    constexpr auto res = cn::f64{42.0}.try_as<std::int32_t>();
    ASSERT_TRUE(res.has_value());
    EXPECT_EQ(res->value(), 42);
}

TEST(Cast, TryAsFloatToIntFractional) {
    constexpr auto res = cn::f64{3.14}.try_as<std::int32_t>();
    EXPECT_FALSE(res.has_value());
}

TEST(Cast, TryAsSameType) {
    constexpr auto res = cn::i32{42}.try_as<std::int32_t>();
    ASSERT_TRUE(res.has_value());
    EXPECT_EQ(res->value(), 42);
}
